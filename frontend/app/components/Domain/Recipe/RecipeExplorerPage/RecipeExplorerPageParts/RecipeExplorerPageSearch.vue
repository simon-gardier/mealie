<template>
  <div class="search-container pb-8">
    <form class="search-box bistro-discovery-panel pa-2" @submit.prevent="search">
      <div class="bistro-discovery-heading">
        <div>
          <span class="bistro-discovery-kicker">Petit Chef</span>
          <h1 class="bistro-discovery-title">
            {{ $t("general.recipes") }}
          </h1>
        </div>
        <v-icon class="bistro-discovery-mark" aria-hidden="true">
      ...
      <div class="search-row">
        ...
        <v-menu offset-y bottom start nudge-bottom="3" :close-on-content-click="false">
          ...
          <v-card>
            <v-card-text>
              <v-switch v-model="state.auto" :label="$t('search.auto-search')" single-line color="primary" />
