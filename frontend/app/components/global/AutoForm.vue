    ...
    <v-card :color="color" :dark="dark" flat :width="width" class="my-2">
      <v-row no-gutters>
        <template v-for="(inputField, index) in items" :key="index">
          <v-col v-if="inputField.section" :cols="12" class="px-2">
          ...
          <v-col :cols="inputField.cols || 12" class="px-2">
            <!-- Check Box -->
            <v-checkbox v-if="inputField.type === fieldTypes.BOOLEAN" v-model="model[inputField.varName]"
              :name="inputField.varName" :readonly="fieldState[inputField.varName]?.readonly"
              :disabled="fieldState[inputField.varName]?.disabled" :hint="inputField.hint"
              :hide-details="!inputField.hint" :persistent-hint="!!inputField.hint" density="comfortable"
              validate-on="input">
              <template #label>
                <span class="ml-4">
                  {{ inputField.label }}
            ...
            <v-number-input v-else-if="inputField.type === fieldTypes.NUMBER" v-model="model[inputField.varName]"
              ...
              :hide-details="!inputField.hint" :persistent-hint="!!inputField.hint"
              :rules="!(inputField.disableUpdate && updateMode) ? inputField.rules || [] : []" validate-on="input" />
